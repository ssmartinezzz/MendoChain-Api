...
## Tasks
- [ ] R1 Contract: `revoke_actor` (admin only, registered accounts); revoked or unregistered accounts cannot send.
- [ ] R2 Ledger port, fake and Algorand adapter: `revoke_actor`; LocalNet test.
- [ ] R3 Domain: `Actor.revoked_at`; `onboard_member` and `revoke_actor` services; revoked actors excluded from recipients and rejected early.
